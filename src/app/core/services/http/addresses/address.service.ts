...
export class AddressService {
  ...
  private securedAddressUrl = environment.API_SECURED_ENDPOINT + '/address';

  findAll(): Observable<Address[]> {
    const endpoint = this.securedAddressUrl.concat('/findAll');
    return this.httpClient.get<Address[]>(endpoint);
  }

  findByCustomerAndType(findByTypeDto: FindByTypeDto): Observable<Address[]> {
    const endpoint = this.securedAddressUrl.concat('/findByCustomerIdAndType');

    return this.httpClient.post<Address[]>(endpoint, findByTypeDto);
  }

  save(addressDto: AddressDto): Observable<Address[]> {
    const endpoint = this.securedAddressUrl.concat('/save');

    return this.httpClient.post<Address[]>(endpoint, addressDto);
  }

  update(addressDto: AddressDto): Observable<Address[]> {
    const endpoint = this.securedAddressUrl.concat('/update');

    return this.httpClient.post<Address[]>(endpoint, addressDto);
  }

  delete(id: number): Observable<Object> {
    const endpoint = this.securedAddressUrl.concat('/delete/').concat(id.toString());
